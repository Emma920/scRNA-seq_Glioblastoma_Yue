...
pre.processing.patient <- function(sce) {
  ...
  dec.block.total <- modelGeneVar(sce.total,  block=sce.total$patient_id) #dec.total considering plate_id
  hvg.block <- getTopHVGs(dec.block.total, var.threshold=0) # hvg considering plate_id
  sce.block.hvg <- sce.total[hvg.block,] #sce.hvg considering plate_id
  head(dec.block.total[order(dec.block.total$bio, decreasing=TRUE),1:6]) # check dec.block.total
