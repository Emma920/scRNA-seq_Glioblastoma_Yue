...
selected.stem.genes <- subset(all.stem.genes, frequencies > 10&pval_rank_median<50)
...
#  geom_text_repel(data=all.stem.genes %>% filter(names %in% membrane), box.padding = unit(1, 'lines'),                   aes(pval_rank_median,frequencies,label=names))+
  ...
  geom_text_repel(data=all.stem.genes %>% filter(names %in% DEGs_up_membran_intersect), box.padding = unit(1, 'lines'),                   aes(pval_rank_median,frequencies,label=names))+
  geom_text_repel(data=all.stem.genes %>% filter(names %in% DEGs_up2_membran_intersect), box.padding = unit(1, 'lines'),                   aes(pval_rank_median,frequencies,label=names))+
  ...
  geom_point(data=all.stem.genes %>% filter(names %in% DEGs_up_membran_intersect), aes(pval_rank_median, frequencies),  shape=16, size=4, color="#ffa500ff")+
  geom_point(data=all.stem.genes %>% filter(names %in% DEGs_up2_membran_intersect), aes(pval_rank_median, frequencies),  shape=16, size=4, color="#a05a2cff")+    
  geom_point(data=all.stem.genes %>% filter(names %in% DEGs_excluded_membran_intersect), aes(pval_rank_median, frequencies),  shape=4, size=4, color="red")+
  geom_point(data=all.stem.genes %>% filter(names %in% DEGs_membrane_intersect), aes(pval_rank_median, frequencies),  shape=2, size=5, color="black")+
...
ggplot() +
  geom_point(data=all.stem.genes, aes(x=pval_rank_median,y=frequencies), , alpha = 0.2) +
  scale_size_continuous(range = c(4,10))+
  geom_text_repel(data=all.stem.genes %>% filter(names %in% DEGs_excluded_selected2), box.padding = unit(1, 'lines'),                   aes(pval_rank_median,frequencies,label=names))+
  #geom_text_repel(data=subset(stem.DEG.all, !is.na(stem.DEG.all$frequencies_intersect)&stem.DEG.all$frequencies>10&stem.DEG.all$pval_rank_median<50), box.padding = unit(1, 'lines'),                   aes(pval_rank_median,frequencies,label=names)) +
  ...
  theme(axis.title.x = element_blank(),
        ...
        plot.title = element_text(hjust = 0.5,size=40),
        aspect.ratio = 0.618)+
  ggsave("/home/yue/hdd/yue/data/output/figures/PROM1/find_markers_DEG_selection2_excluded.pdf", width = 8,  height = 6)
```
